...
#' @export
#' @docType methods
#' @aliases lex.div.corpus,kRp.corpus-method
#' @rdname lex.div.corpus-methods
#' @examples
#' \dontrun{
#' myTexts <- kRp.Corpus(dir=file.path("/home","me","textCorpus"))
#' myTexts <- lex.div.corpus(myTexts)
#' }
#' @include 01_class_01_kRp.corpus.R
#' @import koRpus
#' @export
setGeneric("lex.div.corpus", function(corpus, ...) standardGeneric("lex.div.corpus"))

#' @export
#' @rdname lex.div.corpus-methods
setMethod("lex.div.corpus", signature(corpus="kRp.corpus"), function(corpus, ...){
    slot(corpus, "TTR") <- lapply(slot(corpus, "tagged"), function(thisText){
      lex.div(thisText, ...)
    })
    return(corpus)
  }
