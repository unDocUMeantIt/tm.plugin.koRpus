...
abstract: >
  ...
  which are designed to work with complete text corpora in both \kRp{} and `tm` formats. This vignette gives you a quick overview.
vignette: >
...
to analyse complete text corpora in a single step. These classes are also a first step to combine object classes of both, the \kRp{} and `tm` packages.
...
* class `kRp.topicCorpus` holds a list (named by topics) of objects of
    ...
    + class `kRp.corpus`, which in turn contains objects of both \kRp{} and `tm` classes.
...
If you don't need these levels, you can just use the function `simpleCorpus()` to create objects of class `kRp.corpus`. It represents a flat
corpus of texts. To distinguish texts which came from different sources, use the function `sourcesCorpus()`, which will generate sub-corpora for each source
...
Wikipedia^[see the file `tests/testthat/samples/License\_of\_sample\_texts.txt` for details] in its `tests` directory
...
As you can see, `corpusSummary()` fetches a data frame from the object which contains the summarised results of all
