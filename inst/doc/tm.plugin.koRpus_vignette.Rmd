...
Now, if this still confuses you, you can call the wrapper function `read_corpus()` instead, which will decide which one of these classes best suits your data, judging merely from the input.
...
`simpleCorpus()`, `sourcesCorpus()`, `topicCorpus()`, or `read_corpus()`, respectively. The package includes four sample texts taken from
...
As describe above, you can achieve the same result by using the wrapper `read_corpus()`:
